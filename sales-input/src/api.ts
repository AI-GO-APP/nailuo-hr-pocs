...
export async function submitRecord(objectId: string, data: Record<string, any>): Promise<any> {
  const resp = await fetch(API_BASE + (!!(window as any).__IS_EXTERNAL__ ? '/ext' : '') + '/data/objects/' + objectId + '/records', {
    method: 'POST',
    headers: _getHeaders(),
    credentials: 'include',
...
export async function updateRecord(objectId: string, recordId: string, data: Record<string, any>): Promise<any> {
  const resp = await fetch(API_BASE + (!!(window as any).__IS_EXTERNAL__ ? '/ext' : '') + '/data/records/' + recordId, {
    method: 'PATCH',
    headers: _getHeaders(),
    credentials: 'include',
    body: JSON.stringify({ data }),
